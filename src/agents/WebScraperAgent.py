...
class WebScraperAgent(BaseAgent):
    ...
    def _parse_row(self, cells: List) -> Dict[str, Any]:
        ...
        for i, cell in enumerate(cells):
            ...
            if i == 0:
                ...
                researcher["sexo"] = self._infer_gender(text)
            elif i == 1:
    ...
    def _infer_gender(self, nome: str) -> str:
        """
        Infers gender based on name using simple rules.
        
        Returns:
            "Masculino", "Feminino", or "Nao informado"
        """
        if not nome:
        ...
        feminine_endings = ['A', 'IA', 'NA', 'RA', 'TA', 'NA', 'IA', 'NA']
        for ending in feminine_endings:
            if nome_clean.endswith(ending) and len(nome_clean) > 3:
                return "Feminino"
        
        masculine_names = [
            'JOSE', 'MANUEL', 'JOAO', 'ANTONIO', 'FRANCISCO',
            'CARLOS', 'PAULO', 'PEDRO', 'LUIZ', 'MIGUEL',
            'RAFAEL', 'ROBERTO', 'RICARDO', 'ANDRE', 'MARCOS',
            'FERNANDO', 'EDUARDO', 'ALEXANDRE', 'LUCAS', 'GABRIEL'
        ]
        for name in masculine_names:
            if nome_clean.startswith(name):
                return "Masculino"
